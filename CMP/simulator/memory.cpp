...
Memory::Memory(FILE *dimage, int argc, char const *argv[]) : std::vector<unsigned int>(256, 0), disk(256, 0)
{
	std::vector<unsigned int>::iterator it = this->disk.begin();
	std::vector<unsigned int>::iterator iter = this->begin();
...
void Memory::updateCache(unsigned int pAddr, unsigned int index, unsigned int tag, unsigned int blockOffset)
{
...
void Memory::updateTLB(unsigned int tag, unsigned int ppn, int cycle)
{
...
unsigned int Memory::swap_writeBack(unsigned int vAddr)
{
...
int Memory::log2(unsigned int target)
{
	int ans;
	for(ans = -1 ; ans < 32 ; ) {
		if(target == 0) break;
